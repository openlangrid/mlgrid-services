...
parser.add_argument("--infile",type=str, default="/work/sample/ignacio-bazan-lazcano-book-4-final.jpeg")
...
data_path = "/workspace/clip-interrogator/data/"
...
def interrogate(image, models):
    ...
    for model_name in models:
        print(f"Interrogating with {model_name}...", file=sys.stderr)
    ...
    if caption.startswith(medium):
        result = f"{caption} {bests[1][0][0]}, {bests[2][0][0]}, {bests[3][0][0]}, {flaves}"
    else:
        result = f"{caption}, {medium} {bests[1][0][0]}, {bests[2][0][0]}, {bests[3][0][0]}, {flaves}"
    return result
...
result = interrogate(image, models=models)
with open(f"{infile}.result.txt", "w") as f:
    f.write(result)
print(result)
